...
async function run() {
  ...
  try {
    if (!wallet) throw new Error('no wallet');
    const result = await sdk.faucet(wallet.address, 500);
    assert(result.success === true, `faucet failed: ${JSON.stringify(result)}`);
    assert(result.minted === 500, `minted ${result.minted}, expected 500`);
    assert(result.to === wallet.address, 'faucet to mismatch');
    ok('faucet(500 BB)', `minted: ${result.minted}, new_balance: ${result.new_balance}`);
  ...
  try {
    ...
    assert(result.amount === 10, `amount ${result.amount}, expected 10`);
    ...
    ok('transfer(10 BB)', `sig: ${result.signature.slice(0, 12)}…, fromBal: ${result.fromBalance}, toBal: ${result.toBalance}`);
