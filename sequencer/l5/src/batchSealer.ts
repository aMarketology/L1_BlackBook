...
import { getAllBalances } from './coins.js';
 ...
 * Used to skip redundant seals when the slot timer fires but no $BB balance
 ...
 * PoH tick loop and any trade-triggered seal can never run `sealAndSubmit`
...
/**
 * Snapshot all L5 $BB balances, build a Merkle tree, submit the root to L1,
 * and record the sealed batch locally.
 *
 * Called by the PoH tick loop every `config.slotsPerBatch` slots.
 *
 * Concurrency: calls are serialized one-at-a-time via `sealChain`. The
 * redundant-seal guard then naturally skips a second caller when the first
 * already anchored the identical root.
 *
...
async function doSeal(
  ...
  const { root } = buildMerkleTree('L5', entries);
  ...
  sealBatch(db, 'L5', batchId, root, entries.length, slot);
  setSlotWatermark(db, 'L5', slot);
