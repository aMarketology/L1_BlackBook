...
14. [L2 Implementation Status — What's Still Missing](#14-l2-implementation-status--whats-still-missing)
...
## 14. L2 Implementation Status — What's Still Missing

> **Last audited:** March 20, 2026 — comparing `L2_INTEGRATION_GUIDE.md` against the actual L2 codebase.

### Implemented (working)

| Feature | L2 File | Notes |
|---------|---------|-------|
| `VerifyDeposit` gRPC call | `settlement_bridge.rs` | Called in `enter_bb()`, canonical `depositor_wallet` stored |
| `InitContestReserve` gRPC call | `settlement_bridge.rs` | Called in `fund_bb()`, `bb_reserve` sent as `u64` SPL units |
| `SubmitMerkleRoot` gRPC call | `settlement_bridge.rs` | Binary-packed `signed_message` (§7.1), Ed25519 signature |
| `GetContestStatus` gRPC call | `settlement_bridge.rs` | Client method exists |
| Sorted-pair Merkle tree | `layer_2/svm_settlement.rs` | Custom `hash_pair()` / `build_merkle_root()` / `build_proof()` — matches L1 |
| Leaf format `SHA-256(pubkey[32] ++ amount_le8)` | `layer_2/svm_settlement.rs` | Matches §7.2 exactly |
| `GET /proof/:market/:wallet` | `src/main_v3.rs` | Returns hex proof array + `payout_spl` + `payout_bb` |
| Zero-sum local check | `src/main_v3.rs` | `total_deposited == total_payout + house_rake` computed before submission |
| Sequencer key signing | `settlement_bridge.rs` | Ed25519 sign via `L2_SEQUENCER_KEY` env var |
| Proto field tags / types | `generated/prism.settlement.v1.rs` | Aligned to L1 wire format |

### NOT Yet Implemented (must build)

#### 1. `SyncBridge` heartbeat not called anywhere

The `SyncBridge` RPC exists in the proto and generated client, but the L2 server **never calls it**. The guide (§5.5) says to call it periodically to confirm L1 is alive. Without this, the L2 has no way to detect L1 downtime before a critical `SubmitMerkleRoot` call fails.

**What to build:**
- Background task that calls `SyncBridge({ node_id: "l2-dealer" })` every ~30 seconds
- Log `latest_slot` and `uptime_secs` from the response
- Set an internal `l1_healthy: bool` flag on the `Dealer` struct
- Check `l1_healthy` before accepting new entries (optional gating)

---

#### 2. `claim_deadline_slot` not stored or surfaced

When `SubmitMerkleRoot` succeeds, L1 returns `l1_finalized_slot` — but the L2 never computes or stores the 30-day `claim_deadline_slot`. The guide (§10) defines:

```
claim_deadline_slot = l1_finalized_slot + 6_480_000
```

**What to build:**
- After a successful `SubmitMerkleRoot`, compute `claim_deadline_slot` and store it alongside the market
- Expose `claim_deadline_slot` in the `GET /proof/:market/:wallet` response so the frontend can show "X days left to claim"
- Option A: call `GetContestStatus` to get the authoritative `claim_deadline_slot` from L1
- Option B: compute it locally from `l1_finalized_slot + 6_480_000`

---

#### 3. No claim-window expiry handling

The guide (§10) recommends:
- Warn users at 7 days remaining (~1,512,000 slots before deadline)
- Disable the claim UI / stop serving proofs after `claim_deadline_slot`
- Mark contests as `EXPIRED` in the L2 DB

Currently the L2 has no `EXPIRED` status and no slot-based expiry logic. The `/proof` endpoint will happily serve proofs for markets whose claim window has already closed on L1.

**What to build:**
- Periodic job that calls `GetContestStatus` for all `SETTLED` markets
- When L1 returns `status = "EXPIRED"`, update the L2 market status to `Expired`
- Return a clear error from `GET /proof/:market/:wallet` if the claim window is closed, e.g.:
  ```json
  { "error": "Claim window expired", "claim_deadline_slot": 7900000 }
  ```

---

#### 4. `l1_tx_hash` and `l1_slot` not persisted

After `SubmitMerkleRoot` returns, the L2 logs `l1_tx_hash` and `l1_slot` and includes them in the HTTP response, but they are **not stored in redb**. If the server restarts, this data is lost and cannot be recovered without querying L1 again.

**What to build:**
- Store `l1_tx_hash`, `l1_finalized_slot`, and `claim_deadline_slot` in the redb `MARKETS_TABLE` alongside the market record
- Include them in `GET /markets/:id` and `GET /proof/:market/:wallet` responses

---

#### 5. No HTTP fallback for `SubmitMerkleRoot`

The guide (§6.2) describes `POST /escrow/submit-state-root` as an HTTP alternative to the gRPC `SubmitMerkleRoot` RPC. The L2 currently only uses gRPC. If gRPC is unavailable (firewall, port issue), there is no fallback.

**What to build (optional but recommended):**
- If the gRPC `SubmitMerkleRoot` fails with a connection error, retry once via `POST http://L1_HOST:8080/escrow/submit-state-root`
- Same signed message, same binary packing — just wrapped in JSON hex fields instead of proto bytes

---

#### 6. Settlement proofs and payouts lost on restart

`settlement_proofs` and `settlement_payouts` are in-memory `HashMap`s on the `Dealer` struct. A server restart wipes all cached proofs. Winners who haven't claimed yet will get a 404 from `GET /proof/:market/:wallet`.

**What to build:**
- Persist proofs to redb (or disk) after `resolve_bb()` builds the Merkle tree
- On startup, reload proofs for all `SETTLED` (not expired) markets
- Alternative: re-derive proofs on-the-fly from stored winner lists + payout amounts (since the Merkle tree is deterministic)

---

#### 7. `L2_SEQUENCER_PUBKEY` not set on L1

This is an **ops task**, not code. The L1 rejects every `SubmitMerkleRoot` call if `L2_SEQUENCER_PUBKEY` is not configured (returns `503`).

**Steps:**
1. Start the L2 server with `L2_SEQUENCER_KEY=<64 hex chars>`
2. Copy the pubkey logged at startup: `🔑 [L2] Sequencer pubkey: <hex>`
3. Set `L2_SEQUENCER_PUBKEY=<hex>` on the L1 server
4. Restart L1

---

#### 8. `GRPC_URL` port must be `50052`

The L1 settlement gRPC service runs on port `50052` (not `50051` which is the relay port). Verify the `GRPC_URL` env var on the L2 server points to `http://L1_HOST:50052`.
...
|----------|------|-----------------|
| **P0** | #7 — Set `L2_SEQUENCER_PUBKEY` on L1 | All settlements rejected (503) |
| **P0** | #8 — Verify `GRPC_URL` port 50052 | All gRPC calls fail to connect |
| **P1** | #6 — Persist proofs to disk | Winners lose proofs on L2 restart |
| **P1** | #4 — Persist `l1_tx_hash` / `l1_slot` | Audit trail lost on restart |
| **P2** | #2 — Store `claim_deadline_slot` | Frontend can't show "X days left" |
| **P2** | #3 — Claim-window expiry handling | Stale proofs served after window closes |
| **P3** | #1 — `SyncBridge` heartbeat | No L1 health visibility |
| **P3** | #5 — HTTP fallback for settlement | Single transport = single point of failure |
